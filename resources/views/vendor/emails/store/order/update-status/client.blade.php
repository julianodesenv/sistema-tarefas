                            ...
                            <tr>
                                <td style="{{ $fontFamily }} {{ $style['email-body_cell'] }}">
                                    ...
                                    <span style="{{ $style['paragraph'] }}">
                                        <p>A <b>ASSINATURA #{{ $data->id }}</b> encontra-se {{ $data->status->name }}.</p>
                                        {!! $textEmail !!}
                                    </span>
                                    ...
                                    <span style="{{ $style['paragraph'] }}">
                                        ...
                                        <p>
                                            <strong>Número da ASSINATURA:</strong> {{ $data->id }}<br />
                                    ...
                                    @if(isset($data->plans))
                                        <span style="{{ $style['paragraph'] }}">
                                        <p><strong>Esses são o(s) plano(s) de sua assinatura:</strong></p>
                                    </span>
                                        <table style="{{ $style['body_action'] }}" align="center" width="100%" cellpadding="0" cellspacing="0">
                                            @foreach($data->plans as $plan)
                                                <?php
                                                $cover = asset('uploads/plan/plan-not-image.png');
                                                ...
                                                <tr>
                                                    <td align="center" width="30%">
                                                        @if(isset($cover))
                                                            <a href="{{ route('plan.show', ['type_plan' => $plan->plan->typePlan->seo_link, 'type_egg' => $plan->plan->typeEgg->seo_link,'seo_link' => $plan->plan->seo_link]) }}" target="_blank">
                                                                <img width="70%" src="{{ $cover }}" title="{{ $plan->plan->name }}" alt="{{ $plan->plan->name }}" />
                                                            </a>
                                                        @endif
                                                    </td>
                                                    <td align="left">
                                                <span style="{{ $style['paragraph'] }}">
                                                    <p>
                                                        <strong>{{ $plan->plan->name }}</strong><br />
                                                        {{ $plan->plan->observation }}<br />
                                                        {{ $plan->plan->typePlan->name }}<br />
                                                        <hr />
                                    ...
                                    <span style="{{ $style['paragraph'] }}">
                                        <p>
                                            Lembre-se:<br /><br />
                                            • O prazo de entrega indicado no site conta a partir da confirmação do pagamento;<br /><br />
