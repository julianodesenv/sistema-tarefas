    ...
    <div class="page-content">
        <div class="panel">
            <div class="panel-body">
                <?php $route_back = route('system.task.edit', $schedule_id); ?>
                @include('system.task.inc.menu', ['route_id' => $schedule_id])
                @include('system.layouts.form._form_alerts')

                @include('system.task.user._form')
                        ...
                        @foreach($dados as $row)
                            <tr>
                                ...
                                <td data-title="Ação" class="text-center">
                                    <a href="javascript:void(0);" title="Excluir" data-route="{{ route('system.task.user.destroy', $row->id) }}" data-toggle="modal" data-target="#Destroy" class="btnDestroy btn btn-icon btn-default btn-outline"><i class="icon wb-trash" aria-hidden="true"></i></a>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
